...
?>
<h1>Reporte de Corte a Largo</h1>
<?php
...
$q="select t.descrip, t.especie as especie, sum(m.cantidad) as cantidad, sum(m.cantidad*t.volpt) as vol ".
	"FROM repoCL r ".
	"LEFT JOIN movsRepoCL as m ON r.id=m.idRepoCL ".
	"LEFT JOIN actividades as a ON m.actividad=a.clave ".
	"LEFT JOIN tablas as t ON m.idtabla=t.id ".
	"WHERE $rangoFechas AND tipomov='agregar' group by idtabla ORDER BY especie,grueso,ancho,largo";
// tipomov=agregar identifica a la produccion (proceso=2) de corte a largo
// porque en la tabla tambien hay salidas (para transformacion de medidas)
//
